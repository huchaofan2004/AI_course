...
智能体 Agent中的 ReAct 框架：

ReAct 框架的本质，是用统一的自然语言格式，把「思考（Reasoning）」和「行动（Acting）」拉到台面上，

让我们既能让模型自己规划步骤、调用工具，又能清晰地观察和调试整个过程。

从工程视角，它是实现可控、可解释、多步智能体的一个干净范式； 

从研究视角，它提供了一扇窗，让我们能真正「看见」大模型在复杂任务中的决策轨迹。

### A2A2A

```py
"""
Agent-to-Agent (A2A) 协作案例 — LangChain 1.0 + 通义千问 qwen-plus
...
A2A 调用关系（消息以协调器为中心转发，子 Agent 之间不直接通信 = Mediator 模式）：
